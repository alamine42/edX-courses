...
def parsePoint(line):
    ...
    return LabeledPoint(line_items[0], line_items[1:])
...
def calcRMSE(labelsAndPreds):
    ...
    """
    point_total = labelsAndPreds.count()
    squared_error = (labelsAndPreds
                    .map(lambda lp: squaredError(lp[0], lp[1]))
                    .reduce(lambda a, b: a + b))
    return np.sqrt(squared_error / point_total)
...
def gradientSummand(weights, lp):
    ...
    diff_y = weights_transpose.dot(lp.features) - lp.label
    return diff_y*lp.features
...
def linregGradientDescent(trainData, numIters):
    ...
    for i in range(numIters):
        ...
        labelsAndPredsTrain = trainData.map(lambda observation: getLabeledPrediction(w, observation))
        errorTrain[i] = calcRMSE(labelsAndPredsTrain)

        # Calculate the `gradient`.  Make use of the `gradientSummand` function you wrote in (3a).
        # Note that `gradient` sould be a `DenseVector` of length `d`.
        gradient = (trainData
                    .map(lambda observation: gradientSummand(w, observation))
                    .reduce(lambda a, b: a + b))
...
def twoWayInteractions(lp):
    ...
    """
    two_way_interactions = [ product[0]*product[1] for product in itertools.product(lp.features, lp.features) ]
    return LabeledPoint(lp.label, np.append(lp.features, two_way_interactions))
